<template>
    <LayoutBase>
        <title>活动详情</title>
        <Navbar v-show="false"
                arrowLeft>活动详情</Navbar>
        <ActivityDetailImage :src="data.image"
                             :title="data.name"
            ...
            <CellGroup noMargin>
                ...
                <Cell v-if="data.signupCount"
                    ...
                    <div class="activity-card-message">
                        ...
                        <div @click="goSignUps"
                             class="activity-card-message-avatars">
                            <Avatar v-for="i in data.signups"
                                    :src="i.image"
                                    :key="'avatar' + i.id"
                                    size="0.09rem" />
                            <Icon class="activity-card-arrow-right"
                                  @click="signUpMenu"
                                  type="arrow-right"
                                  size="0.05rem" />
                        </div>
        ...
        <Panel noPadding>
            <ActivityCardItem v-if="data.description"
                              title="活动介绍">
                <div v-html="data.description"></div>
            </ActivityCardItem>
            <ActivityCardItem v-if="data.rule_desc"
                              title="规则">
                <div v-html="data.rule_desc"></div>
            </ActivityCardItem>
            <ActivityCardItem v-if="data.site_desc"
                              title="场地">
                <div v-html="data.site_desc"></div>
            </ActivityCardItem>
            <ActivityCardItem title="评论"
                              noBorder>
                <div class="activity-comment-area">
                    <div class="activity-empty-comment"
                         v-if="comments.length === 0">快来发表评论吧</div>
                    <div v-else>
                        <div v-if="!showAllComments && index < 3 || showAllComments"
                             v-for="(i, index) in comments"
                             v-bind:key="'comment_' + i.id"
                            ...
                            <div class="activity-comment-item-content">
                                <div class="activity-comment-item-content-body">
                                    ...
                                    <div class="flexable">
                                        <Icon @click="like(i.id)"
                                              v-if="!i.isPraise"
                                              size="0.055rem"
                                              class="like"
                                              type="unlike" />
                                        <Icon @click="unlike(i.id)"
                                              v-if="i.isPraise"
                                              size="0.055rem"
                                              class="like"
                                              type="like" />
                                        <div class="like-num">{{i.praise}}</div>
                                        <Icon @click="commentTo(i.id, i.userName)"
                                              size="0.06rem"
                                              class="comment-icon"
                                              type="comment-icon" />
                                    </div>
                        ...
                        <div v-if="!showAllComments && comments.length > 3"
                             @click="more_comment"
                             class="more-comment">查看更多评论</div>
                    </div>
                </div>
            </ActivityCardItem>
        </Panel>
        <div v-show="commentToolsVisible"
             @click="hiddenComment"
             class="comment-mask"></div>
        <div v-show="commentToolsVisible"
             class="comment-tools">
            <form action="#"
        ...
        <div class="sign-bar">
            <div class="sign-item-wrap">
                ...
                <Button @onClick="sign_up"
                        ...
                        size="small-padding"
                        circle
                        class="sign-up-button">{{buttonText}}</Button>
            </div>
        </div>
    </LayoutBase>
</template>
...
export default {
  ...
  computed: {
    displayActivityStatus() {
      ...
      if (!data.signup_begin_time) {
        // 没有报名时间，直接根据活动状态返回状态描述
      ...
      const now = new Date();
      const signup_begin_time = moment(data.signup_begin_time).toDate();
      const signup_end_time = moment(data.signup_end_time).toDate();
      const begin_time = moment(data.begin_time).toDate();
      const end_time = moment(data.end_time).toDate();

      console.log(
        now.toISOString(),
        signup_begin_time.toISOString(),
        signup_end_time.toISOString(),
        begin_time.toISOString(),
        end_time.toISOString()
      );

      if (now < signup_begin_time && now < begin_time) {
        // 在报名和活动开始时间之前
        return "未开始";
      } else if (now > end_time && now > signup_end_time) {
        // 在报名和活动结束时间之后
        return "已结束";
      } else {
        return "进行中";
  ...
  methods: {
    ...
    refresh() {
      requests(
        `${API.get_activitys}/${this.$route.params.id}/`,
        {
          type: "GET"
        },
        data => {
          ...
          if (this.data.signup_begin_time) {
            if (
              date.isAfter(moment(this.data.signup_begin_time)) &&
    ...
    refresh_comments() {
      requests(
        `${API.get_activitys}/${this.$route.params.id}/comments/`,
        {
          type: "GET"
        },
        data => {
    ...
    sign_up() {
      if (this.buttonText === "我要报名") {
        requests(
          `${API.get_activitys}/${this.$route.params.id}/signups`,
          {
            type: "POST"
          },
          data => {
