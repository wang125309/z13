<template>
    <LayoutBase>
        ...
        <ActivityCardItem no-padding  cut full title="商品规格" v-if="data.washCarServices">
            <Good v-for="i in data.washCarServices"
                  :key="'good' + i.id"
                  :title="i.name"
                  :logo="i.image"
                  :price="i.price"
...
<script>
    ...
    export default {
        ...
        methods: {
            refresh () {
                request(`${API.washcars}/${this.$route.params.id}/`, {
                    type: 'GET'
                }, (data) => {
                    this.data = data.data;
                }, (data) => {
